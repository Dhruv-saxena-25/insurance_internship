...
class GCloudSync:
    ...
    def sync_folder_to_gcloud(self, gcp_bucket_url, filepath, filename):   # To puch data to gcloud

        command = f"gsutil cp {filepath}/{filename} gs://{gcp_bucket_url}/"
        # command = f"gcloud storage cp {filepath}/{filename} gs://{gcp_bucket_url}/"
        os.system(command)

    def sync_folder_from_gcloud(self, gcp_bucket_url, filename, destination):   # To pull data from gcloud
